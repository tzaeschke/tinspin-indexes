...
/**
 * Main test runner class. The test runner can be executed directly or remotely in a separate
...
public class TestRunner {
  ...
  public static void main(String[] args) {
    // -Xmx28G -XX:+UseConcMarkSweepGC -Xprof -XX:MaxInlineSize=0 -XX:FreqInlineSize=0
    ...
    //		System.out.println("AMM: " + PhIteratorNoGC.AMM1 + " / " + PhIteratorNoGC.AMM2 + " / " +
    // PhIteratorNoGC.AMM3 + " / " + PhIteratorNoGC.AMM4 + " / " + PhIteratorNoGC.AMM5 + " / ");
    //		System.out.println("VMM: " + PhIteratorNoGC.VMM1 + " / " + PhIteratorNoGC.VMM2 + " / " +
    // PhIteratorNoGC.VMM3);
    //		System.out.println("HCI-A/L/N: " + PhIteratorNoGC.HCIA + " / " + PhIteratorNoGC.HCIL + " / "
    // + PhIteratorNoGC.HCIN);
  }
  ...
  /**
   * This method sets the random seed to the default seed plus a given delta. This solves the
   * problem that, for example, the kNN generator would generate the same points as the data
   * generator, which resulted in 0.0 distance for all queried points.
   *
   * @param delta
   */
  ...
  private int getKnnRepeat(int dims) {
    if ((S.TEST == TestInstances.TST.CLUSTER_P || S.TEST == TestInstances.TST.CLUSTER_R)
        && S.cfgNDims > 5) {
  ...
  private void repeatQuery(int repeat, int round) {
    ...
    log(
        "Query time: "
            + toMS(t1, t2)
            + " ms -> "
            + toMS(t1, t2) / (double) repeat
            + " ms/q -> "
            + toNSPerOp(t1, t2, repeat)
            + " ns/q/r  (n="
            + n
            + ")"
            + "; total queries: "
