...
public abstract class AbstractTest {
  ...
  public void generateWindowQueries(double[][] lower, double[][] upper) {
    ...
    // final double qVolume = 0.01 * Math.pow(cfgDataLen, DIM);//(float) Math.pow(0.1, DIM); //0.01
    // for DIM=2
    double totalV = 1;
    for (int i = 0; i < DIM; i++) {
  ...
  /**
   * Generate query rectangle. This method should be overwritten by tests that provide non-standard
   * queries.
   *
