...
/**
 * CLUSTER 3.4/3.5 are introduced by: [1] L. Arge, M. de Berg, H. J. Haverkort and K. Yi: "The
...
public class TestPointCluster extends TestPoint {
   ...
   *
   * <p>From [1]: cluster: Our final dataset was designed to illustrate the worst-case behavior of
   * the H, H4 and TGS R-trees. It is similar to the worst-case example discussed in Section 2. It
   * consists of 10 000 clusters with centers equally spaced on a horizontal line. Each cluster
   * consists of 1000 points uniformly distributed in a 0.000 01 * 0.000 01 square surrounding its
   * center.
   *
   * @return Elements
   */
  ...
  public double[] generate() {
    ...
    switch (TYPE.toType(param1)) {
      case ORIGINAL:
        return generateOriginal(len);
      case HORIZONTAL05:
        return generateHorizontal(len, 0.5);
      case HORIZONTAL04:
        return generateHorizontal(len, 0.4);
      case HORIZONTAL001:
        return generateHorizontal(len, 0.01);
      case DIAGONAL:
        return generateDiagonal(len);
      case GAUSS:
        return generateGauss(len);
    }
    ...
    for (int c = 0; c < BOX_N; c++) {
      ...
      for (int p = 0; p < N_C; p++) {
        ...
        for (int d = 0; d < DIM; d++) {
          ...
          // data[ii + d] += 0.00005;  TODO difference between 1.5 and 9.9 million nodes per 10
          // million entries (for offs=0.5).
          if (d == 0) {
            data[ii + d] += x0;
          } else {
            data[ii + d] += yz0;
  ...
  @Override
  public void generateQuery(double[] min, double[] max, final double maxLen, final double avgQVol) {
    switch (TYPE.toType(param1)) {
      case ORIGINAL:
        queryCuboidOld(min, max);
        break;
      case HORIZONTAL04:
      case HORIZONTAL05:
        queryCuboidHorizontalNormal(min, max);
        break;
      case HORIZONTAL001:
        queryCuboidHorizontal001(min, max);
        break;
      case DIAGONAL:
        queryCuboidDiag(min, max);
        break;
      case GAUSS:
        super.generateQuery(min, max, maxLen, avgQVol);
        break;
      default:
        throw new IllegalArgumentException("param1=" + param1);
    }
  ...
  public double maxUpdateDistance() {
    switch (TYPE.toType(param1)) {
      ...
      case HORIZONTAL05:
        return BOX_LEN / 100;
      case GAUSS:
        return super.maxUpdateDistance(); // GAUSS_SIGMA/10;
      default:
        throw new IllegalArgumentException("param1=" + param1);
    }
  }
