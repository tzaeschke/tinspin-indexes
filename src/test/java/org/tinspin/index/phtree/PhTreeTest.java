  ...
  private void repeatQuery(TestBox test, BoxMap<Integer> tree1, BoxMap<Integer> tree2, int repeat) {
    ...
    println(
        "Query time: "
            ...
            + (t2 - t1) / (double) repeat
            + " ms/q -> "
            + (t2 - t1) * 1000 * 1000 / (double) n
            + " ns/q/r  (n="
            + n
            + ")");
  }

  private int repeatQueries(
      BoxMap<Integer> tree1, BoxMap<Integer> tree2, double[][] lower, double[][] upper) {
    int n = 0;
    for (int i = 0; i < lower.length; i++) {
      ...
      if (n1 != n2) {
        ...
        for (int j = 0; j < set1.size(); j++) {
          ...
          if (!Arrays.equals(e1.min(), e2.min()) || !Arrays.equals(e1.max(), e2.max())) {
