...
public class TestCritBit {
  ...
  public void testBugInsert1() {
    long[] a = new long[] {-723955400, -1690734402, -1728529858, -1661998771};
  ...
  public void testBugInsert2() {
    long[] a = new long[] {-65105105, -73789608, -518907128};
    CritBit1D<Integer> cb = newCritBit(32);
    for (int i = 0; i < a.length; i++) {
      a[i] = a[i] << 32;
      // System.out.println("Inserting: " + a[i] + " / " + BitsInt.toBinary(a[i] >>> 32));
  ...
  public void testIteratorWithMask() {
    ...
    // test
    QueryIteratorWithMask<Integer> it =
        new QueryIteratorWithMask<>((CritBit<Integer>) cb, minMask, maxMask, k);
    int n = 0;
    while (it.hasNext()) {
      Entry<Integer> e = it.nextEntry();
      assertNotNull(e);
  ...
  private boolean doesInfixMatch(long[] currentVal, int posDiff, long[] v, int startSlot) {
    //		if (n.infix == null) {
    //			return true;
    //		}
