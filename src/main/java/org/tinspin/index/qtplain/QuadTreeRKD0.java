...
public class QuadTreeRKD0<T> implements BoxMap<T>, BoxMultimap<T> {
   ...
   * @param <T> Value type
   */
  public static <T> QuadTreeRKD0<T> create(
      double[] min, double[] max, boolean align, int maxNodeSize) {
    double radius = 0;
  ...
  public BoxIterator<T> queryExactBox(double[] lower, double[] upper) {
    ...
    return new BoxIteratorWrapper<>(
        lower,
        upper,
        (low, upp2) -> {
          ArrayList<BoxEntry<T>> results = new ArrayList<>();
          // Hack: we use the 'condition' to collect results, however, in order to continue search,
          // we return `false`.
          if (root != null) {
  ...
  public boolean update(
      double[] oldKeyL, double[] oldKeyU, double[] newKeyL, double[] newKeyU, T value) {
    if (root == null) {
      return false;
    }
  ...
  public String toString() {
    return "QuadTreeRKD0;maxNodeSize="
        + maxNodeSize
        + ";maxDepth="
        + MAX_DEPTH
        + ";DEBUG="
