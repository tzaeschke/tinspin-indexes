...
public class KDTree<T> implements PointMap<T>, PointMultimap<T> {
  ...
  private Node<T> findNodeExact(
      double[] key, RemoveResult<T> resultDepth, Predicate<PointEntry<T>> filter) {
    if (root == null) {
      return null;
    }
  ...
  private Node<T> findNodeExactFast(
      double[] key, Node<T> parent, RemoveResult<T> resultDepth, Predicate<PointEntry<T>> filter) {
    Node<T> n = root;
    do {
  ...
  private Node<T> findNodeExactSlow(
      double[] key,
      Node<T> n,
      Node<T> parent,
      RemoveResult<T> resultDepth,
      Predicate<PointEntry<T>> filter) {
    do {
      double[] nodeKey = n.point();
      double nodeX = nodeKey[n.getDim()];
      double keyX = key[n.getDim()];
  ...
  private double rangeSearchKNN(
      Node<T> node,
      double[] center,
      ArrayList<PointEntryKnn<T>> candidates,
      int k,
      double maxRange,
  ...
  public String toString() {
    return "KDTree;size="
        ...
        + DEBUG
        + ";center="
        + (root == null ? "null" : Arrays.toString(root.point()));
  }
