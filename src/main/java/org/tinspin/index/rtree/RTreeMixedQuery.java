...
class RTreeMixedQuery<T> implements Iterator<BoxEntryKnn<T>> {
  ...
  private static class RTreeNodeWrapper<T> extends BoxEntryKnn<T>
  ...
  /**
   * Subclass which holds a reference to the parent node for optimized support of the
  ...
  public RTreeMixedQuery(
      RTree<T> tree, double[] center, Filter filter, BoxDistance dist, BoxDistance closestDist) {
    this.tree = tree;
    this.center = center;
  ...
  @Override
  public String toString() {
    return "RTreeMixedQuery [queueSize="
        + queueSize()
        + ", rm.loss="
        + remove_pointerLoss
        + ", rm.hit="
