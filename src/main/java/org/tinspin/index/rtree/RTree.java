...
/**
 * R*Tree implementation based on the paper from Beckmann, N.; Kriegel, H. P.; Schneider, R.;
 * Seeger, B. (1990). "The R*-tree: an efficient and robust access method for points and
 * rectangles".
 *
 * <p>Revised R*Tree (for DBMS, see conclusion?) -- RR*Tree "A Revised R*-tree in Comparison with
 * Related Index Structures" Norbert Beckmann; Bernhard Seeger
 *
 * @author ztilmann
 * @param <T> Value type.
 */
public class RTree<T> implements BoxMap<T>, BoxMultimap<T> {
  ...
  /**
   * @param e entry
   * @param desiredInsertionLevel Entries have to be inserted at the desired level. The level is
   *     usually '0' (for data points) but can be higher for reinsertion of subtrees.
   */
  private void insert(RTreeEntry<T> e, boolean[] blockedLevels, int desiredInsertionLevel) {
    ...
    if (logic.hasSpace(node)) {
      ...
      RTreeNode<T> newNode = overflowTreatment(node, e, blockedLevels, desiredInsertionLevel);
  ...
  private RTreeNode<T> overflowTreatment(
      RTreeNode<T> node, RTreeEntry<T> e, boolean[] blockedLevels, int desiredInsertionLevel) {
    // OT1
  ...
  public boolean contains(double[] min, double[] max, T value) {
    return findNodeEntry(min, max, (entry, node, posInNode) -> Objects.equals(value, entry.value()))
        != null;
  }
  ...
  private boolean findNodes(
      double[] min, double[] max, RTreeNode<T> node, CheckNodeAbort<T> checkAbort) {
    if (node instanceof RTreeNodeDir) {
      ArrayList<RTreeNode<T>> children = ((RTreeNodeDir<T>) node).getChildren();
      for (int i = 0; i < children.size(); i++) {
        RTreeNode<T> sub = children.get(i);
  ...
  public Iterable<BoxEntryKnn<T>> queryRangedNearestNeighbor(
      ...
      double[] minBound,
      double[] maxBound) {
    return queryRangedNearestNeighbor(
        center, dist, closestDist, new Filter.RectangleIntersectFilter(minBound, maxBound));
  }

  /**
   * This method returns an Iterable which returns the nodes by a combined range and nearest number
   * search. The Iterator supports the {@code Iterator.remove()} method.
   *
   * @param center Target position passed as parameter to the distance functions. Can be {@code
   *     null} if your distance function supports it (like {@code
   *     RectangleDistanceFunction.RectangleDist}).
   * @param dist Distance function used to compare entries (example: {@code
   *     RectangleDistanceFunction.EDGE} or {@code RectangleDistanceFunction.CENTER})
   * @param closestDist Distance of the best point in a given rectangle (example: {@code
   *     RectangleDistanceFunction.EDGE} but *not* {@code RectangleDistanceFunction.CENTER})
   * @param filter Filter to limit the results for range queries (example: {@code new
   *     Filter.RectangleIntersectFilter(min, max)})
   * @return An Iterable which lazily calculates the nearest neighbors.
   */
  public Iterable<BoxEntryKnn<T>> queryRangedNearestNeighbor(
      double[] center, BoxDistance dist, BoxDistance closestDist, Filter filter) {
    RTree<T> self = this;
  ...
  private void toStringTree(StringBuilderLn sb, RTreeNode<T> node, int level) {
    ...
    sb.appendLn(
        pre
            ...
            + node.toString()
            + ";P="
            + System.identityHashCode(node.getParent()));
