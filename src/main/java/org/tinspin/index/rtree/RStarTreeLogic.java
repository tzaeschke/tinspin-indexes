...
public class RStarTreeLogic implements RTreeLogic {
  ...
  @Override
  public <T> RTreeNode<T> chooseSubTree(
      RTreeNode<T> root, RTreeEntry<T> e, int desiredInsertionLevel, int nLevels) {
    // CS1
  ...
  private <T> double calcOverlapSize(
  ...
  private <T> RTreeNode<T> chooseNodeWithLeastAreaEnlargement(
      RTreeNodeDir<T> dir, RTreeEntry<T> e) {
    ArrayList<RTreeNode<T>> children = dir.getChildren();
   ...
   * Choose subtree as described in the paper.
   *
   * <p>According to paper the function a) returns (param) a sorted array of BB sorted for splitting
   * b) returns (ret) the split dimension c)
   */
  @SuppressWarnings("unchecked")
  @Override
  ...
  private <T> double calcMargin(
      RTreeEntry<T>[] entries, int start, int end, double[] min, double[] max) {
