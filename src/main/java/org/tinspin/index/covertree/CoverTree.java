 ...
 *
 * <p>"Faster Cover Trees", Mike Izbicki, Christian R. Shelton, Proceedings of the 32nd
 * International Conference on Machine Learning, Lille, France, 2015.
 *
 * <p>Changes over original algorithms: - findNearestNeighbour compares if (d(y, x) greater (d(_x_,
 * q.point()) - q.maxdist(this))) instead of if (d(y, x) greater (d(_y_, q.point()) -
 * q.maxdist(this))) - maxDist: - lazily calculated. - calculated while excluding subbranches that
 * don't need to be calculated - Invalidated on modifications (should speed up remove/insert) -
 * Algorithm 2 (insert) uses BASE=1.3: BASE*covdist(p) instead of 2*covdist(p)) - Rebalancing: - The
 * paper proposes to sort the children on their distance to 'x' and then insert into the first one
 * that has enough covdist. The sorting appears unnecessary because we anyway have to check all
 * children, and finding the closest one is trivially possible without sorting. - Some optimizations
 * for kNN which are not discussed in the paper
 *
 * <p>Other: - We also implemented the kNN algorithm by Hjaltason and Samet, but it about 2x as long
 * as our own algorithm in all scenarios we tested. TODO - Rebalancing (nearest-ancestor) - Merge
 * Node/Point classes
 *
...
public class CoverTree<T> implements PointMap<T> {
   ...
   * @param <T> value type
   */
  public static <T> CoverTree<T> create(PointEntry<T>[] data, double base, PointDistance distFn) {
  ...
  private Node<T> rebalanceTZ(Node<T> p, Node<T> x, double distPX) {
    ...
    if (p.hasChildren()) {
      ...
      for (int i = 0; i < children.size(); i++) {
        ...
        if (q.hasChildren()) {
          double distQX = d(q, x);
          if (distQX <= 2 * covDist && distQX <= covDist + q.maxdist(this)) {
  ...
  private Node<T> rebalance(
      Node<T> p, Node<T> q, Node<T> x, ArrayList<Node<T>> moveSet, ArrayList<Node<T>> staySet) {
  ...
  private void findNearestNeighbor(
      Node<T> p, PointEntry<T> x, final PointEntryKnn<T> y, double distPX) {
  ...
  private void findNearestNeighbor(
      Node<T> p, double[] x, int k, ArrayList<PointEntryKnn<T>> candidates, double distPX) {
    //		Algorithm 1 Find nearest neighbor
    //		function findNearestNeighbor(cover tree p, query
    //		point x, nearest neighbor so far y)
    //		1: if d(p;x) < d(y;x) then
      ...
      if (maxDist != node.maxdist(this)) {
        throw new IllegalStateException(
            "Maxdist: "
                ...
                + node.maxdist(this)
                + " Node:"
                + toStringNode(new StringBuilder(), node));
      }
      if (maxDist > covdist(node)) {
        throw new IllegalStateException(
            "Maxdist/maxdist()/CovDist: "
                + maxDist
                + " / "
                + node.maxdist(this)
                + " / "
  ...
  public static class CTStats extends Stats {
    ...
    public String toString() {
      return super.toString() + ";sumMaxDist=" + sumMaxDist;
    }
  }
