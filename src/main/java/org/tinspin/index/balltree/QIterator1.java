...
public class QIterator1<T> implements PointIterator<T> {
	...
	private class IteratorStack {
		...
		StackEntry<T> prepareAndPush(BTNode<T> node, double[] min, double[] max) {
			if (size == stack.size()) {
	...
	private static class StackEntry<T> {
		...
		void set(BTNode<T> node, double[] min, double[] max) {
	...
	QIterator1(BallTree<T> tree, double[] min, double[] max) {
		...
		while(!stack.isEmpty()) {
			...
			while (se.pos < se.len) {
				...
				if (se.isLeaf()) {
					...
					if (BTUtil.isPointEnclosed(e.point(), min, max)) {
						next = e;
						return;
					}
				} else {
