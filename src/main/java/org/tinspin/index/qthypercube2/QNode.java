 ...
 *
 * <p>A node can be in one of two modes: "directory node" and "leaf node". Directory nodes have an
 * 2^dim array of "subs", where ich entry can be one of: a point, a subnode, or null. Leaf nodes
 * have an array of "values" which are all points.
 *
 * <p>A new subnode is inserted in "subs" if a slot in "subs" already contains a point.
 *
 * @author ztilmann
 * @param <T> Value type.
 */
public class QNode<T> {
  ...
  private boolean areAllPointsIdentical(PointEntry<T> e) {
    // This discovers situation where a node overflows, but splitting won't help because all points
    // are identical
  ...
  /**
   * The subnode position has reverse ordering of the point's dimension ordering. Dimension 0 of a
   * point is the highest ordered bit in the position.
   *
   * @param p point
  ...
  @SuppressWarnings("unchecked")
  PointEntry<T> remove(
      QNode<T> parent, double[] key, int maxNodeSize, Predicate<PointEntry<T>> pred) {
    if (!isLeaf()) {
