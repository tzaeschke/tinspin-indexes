...
/**
 * This is a MX-quadtree implementation with configurable maximum depth, maximum nodes size, and (if
 * desired) automatic guessing of root rectangle.
 *
 * <p>For navigation during insert/delete/update/queries, it uses hypercube navigation as described
 * by T. Zaeschke and M. Norrie, "Efficient Z-Ordered Traversal of Hypercube Indexes, BTW
 * proceedings, 2017.
 *
 ...
 *
 * <p>This version 2 of the QuadtreeHC reduces memory consumption allowing directory nodes to
 * contain data entries. With version 1, there were many nodes with just one data entry because the
 * parent directory node could not hold date entries. With version two a directory node (which
 * contains a hypercube array) will only create a subnode if a quadrant has to hold more than one
 * data entry.
 *
 * @author ztilmann
 * @param <T> Value type.
...
public class QuadTreeKD2<T> implements PointMap<T>, PointMultimap<T> {
   ...
   * @param <T> Value type
   */
  public static <T> QuadTreeKD2<T> create(
  ...
  private void adjustRootSize(double[] key) {
    ...
    if (root.getRadius() == INITIAL_RADIUS) {
      ...
      if (radius > 0) {
        ...
        // all entries have (approximately?) the same coordinates. We just set an arbitrary radius
        // here.
  ...
  public String toString() {
    return "QuadTreeKD2;maxNodeSize="
        + maxNodeSize
        + ";maxDepth="
        + MAX_DEPTH
        + ";DEBUG="
  ...
  public static class QStats extends Stats {
    ...
    public String toString() {
      return super.toString()
          + ";\n"
          + "histoVal:"
          + Arrays.toString(histoValues)
          + "\n"
          + "histoSub:"
          + Arrays.toString(histoSubs);
    }
  }
