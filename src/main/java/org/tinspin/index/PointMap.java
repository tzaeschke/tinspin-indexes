...
/**
 * A common interface for spatial indexes (maps) that use points as keys. This interface is somewhat
 * inconsistent because it suggests that implementations act as "maps" which means that a given keys
 * can exist only once and is overwritten when a new entry with the same key is added.
 *
 * <p>However, most implementations in this library (except for the PH-Tree) act as multimaps which
 * means they allow multiple entries with identical keys.
 *
...
public interface PointMap<T> extends Index {
  ...
  /**
   * Finds the nearest neighbor. This uses Euclidean distance. Other distance types can only be
   * specified directly on the index implementations.
   *
  ...
  interface Factory {
    /**
     * Create an array backed PointMap. This is only for testing and rather inefficient for large
     * data sets.
     *
    ...
    /**
     * WARNING: Unaligned center and radius can cause precision problems, see README. Create a plain
     * Quadtree. Center/radius are used to find a good initial root. They do not need to be exact.
     * If possible, they should span an area that is somewhat larger rather than smaller than the
     * actual data.
     *
     * @param dims Number of dimensions.
     ...
     */
    @Deprecated
    static <T> PointMap<T> createQuadtree(
        int dims, int maxNodeCapacity, double[] center, double radius) {
      return QuadTreeKD0.create(dims, maxNodeCapacity, center, radius);
    }

    /**
     * Create a plain Quadtree. Center/radius are used to find a good initial root. They do not need
     * to be exact. If possible, they should span an area that is somewhat larger rather than
     * smaller than the actual data.
     *
     * <p>Center and radius will be aligned with powers of two to avoid precision problems.
     *
     * @param center Estimated center of all coordinates.
     * @param radius Estimated maximum orthogonal distance from center for all coordinates.
    ...
    /**
     * WARNING: Unaligned center and radius can cause precision problems, see README. Create a
     * Quadtree with hypercube navigation. Center/radius are used to find a good initial root. They
     * do not need to be exact. If possible, they should span an area that is somewhat larger rather
     * than smaller than the actual data.
     *
     * @param dims Number of dimensions.
     * @param maxNodeCapacity Maximum entries in a node before the node is split. The default is 10.
     * @param center Estimated center of all coordinates.
     ...
     */
    @Deprecated
    static <T> PointMap<T> createQuadtreeHC(
        int dims, int maxNodeCapacity, double[] center, double radius) {
      return QuadTreeKD.create(dims, maxNodeCapacity, center, radius);
    }

    /**
     * Create a Quadtree with hypercube navigation. Center/radius are used to find a good initial
     * root. They do not need to be exact. If possible, they should span an area that is somewhat
     * larger rather than smaller than the actual data.
     *
     * <p>Center and radius will be aligned with powers of two to avoid precision problems.
     *
     * @param center Estimated center of all coordinates.
    ...
    /**
     * WARNING: Unaligned center and radius can cause precision problems, see README. Create a
     * Quadtree with extended hypercube navigation. Center/radius are used to find a good initial
     * root. They do not need to be exact. If possible, they should span an area that is somewhat
     * larger rather than smaller than the actual data.
     *
     * @param dims Number of dimensions.
     ...
     */
    @Deprecated
    static <T> PointMap<T> createQuadtreeHC2(
        int dims, int maxNodeCapacity, double[] center, double radius) {
      return QuadTreeKD2.create(dims, maxNodeCapacity, center, radius);
    }

    /**
     * Create a Quadtree with extended hypercube navigation. Center/radius are used to find a good
     * initial root. They do not need to be exact. If possible, they should span an area that is
     * somewhat larger rather than smaller than the actual data.
     *
     * <p>Center and radius will be aligned with powers of two to avoid precision problems.
     *
     * @param center Estimated center of all coordinates.
     ...
     *
     * @param dims Number of dimensions.
     * @param entries All entries of the tree. Entries can be created with {@link
     *     RTreeEntry#createPoint(double[], Object)}
     * @param <T> Value type
