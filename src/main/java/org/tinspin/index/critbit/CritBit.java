 ...
 *
 * <p>Cribit trees are very space efficient due to prefix-sharing and suitable for multi-dimensional
 * data with low dimensionality (e.g. less than 10 dimensions or so). They are also stable, that
 * means unlike kD-trees or quadtrees they do not require rebalancing, this makes update performance
 * much more predictable.
 *
 * <p>There is 1 1D-version and a kD-version (kD: k-dimensional). The 1D version supports keys with
 * arbitrary length (e.g. 256bit), the kD-version supports k-dimensional keys with a maximum length
 * of 64 bit per dimension.
 *
 * <p>Both tree versions use internally the same methods, except for the range queries. For range
 * queries, the 1D version interprets the parameters as one minimum and one maximum value. For kD
 * queries, the parameters are interpreted as arrays of minimum and maximum values (i.e. the low
 * left and upper right corner of a query (hyper-)rectangle).
 *
 * <p>All method ending with 'KD' are for k-dimensional use of the tree, all other methods are for
 * 1-dimensional use. Exceptions are the size(), printTree() and similar methods, which work for all
 * dimensions.
 *
 * <p>In order to store floating point values, please convert them to 'long' with
 * BitTools.toSortableLong(...), also when supplying query parameters. Extracted values can be
 * converted back with BitTools.toDouble() or toFloat().
 *
 * <p>Version 1.3.5 - Fixed rare problem with postfix creation. This solves a problem with
 * kd-queries and slightly reduces memory consumption.
 *
 * <p>Version 1.3.2 - Added QueryIterator.reset()
 *
 * <p>Version 1.3.1 - Fixed issue #3 where iterators won't work with 'null' as values.
 *
 * <p>Version 1.2.2 - Moved tests to tst folder
 *
 * <p>Version 1.2.1 - Replaced compare() with isEqual() where possible - Simplified compare(),
 * doesInfixMatch() - Removed unused arguments
 *
 * <p>Version 1.2 - Added iterator() to iterate over all entries
 *
 * <p>Version 1.1 - Slight performance improvements in mergeLong() and readAndSplit()
 *
...
public class CritBit<V> implements CritBit1D<V>, CritBitKD<V> {
    ...
    while (true) {
      ...
      if (n.infix != null) {
        ...
        if (posDiff < n.posDiff && posDiff != -1) {
          ...
          // new sub-node
          Node<V> newSub =
              new Node<V>(posDiff + 1, n.loPost, n.loVal, n.hiPost, n.hiVal, subInfix, n.posDiff);
          newSub.hi = n.hi;
          newSub.lo = n.lo;
  ...
  private static long[] extractInfix(long[] v, int startPos, int endPos) {
    ...
    // System.out.println("s/e/l/sp/ep=" + start + "/" + end + "/" + inf.length + "/" + startPos +
    // "/" + endPos);
  ...
  private void updateParentAfterRemove(
      Node<V> parent,
      long[] newPost,
      V newVal,
      Node<V> newSub,
  ...
  public static class QueryIteratorWithMask<V> implements Iterator<V> {
    ...
    private boolean checkMatch(long[] keyTemplate, int startBit, int currentDepth) {
      ...
      if (toCheck != 0) {
        ...
        // if ((diffLo | localDiffLo) != diffLo || (diffHi | localDiffHi) != diffHi) {
        boolean r =
            ((((diffLo | localDiffLo) ^ diffLo) | ((diffHi | localDiffHi) ^ diffHi)) & mask) == 0;
        if (r) {
  ...
  public static class QueryIteratorKD<V> implements Iterator<V> {
    ...
    /**
     * Full comparison on the parameter. Assigns the parameter to 'nextKey' if comparison fits.
     *
    ...
    private boolean checkMatchOrigKD(long[] keyOrigTemplate, int currentDepth) {
      ...
      int commonBits =
          (currentDepth + 1) / DIM; // getDepthAcrossDims(currentDepth);//currentDepth / DIM;
    ...
    /**
     * Calculate the common minimum depth across all dimensions. This is equal to {@code
     * floor(posDirstBit/DIM)}.
     *
