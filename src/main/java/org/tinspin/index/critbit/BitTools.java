...
public class BitTools {
  ...
  public static double[] toDouble(long[] value, double[] ret) {
    for (int i = 0; i < value.length; i++) {
      ret[i] = Double.longBitsToDouble(value[i] >= 0.0 ? value[i] : value[i] ^ 0x7FFFFFFFFFFFFFFFL);
    }
    return ret;
  }
  ...
  /**
   * Splits a value and write it to trgV at position trg1 and trg2. This is the inverse operation to
   * merge(...).
   *
  ...
  public static void split(
      final long toSplit, long[] trgV, final int trg1, final int trg2, int nBits) {
  ...
  /**
   * Merges two long values into a single value by interleaving there respective bits. This is the
  ...
  /**
   * Merges long values into a single value by interleaving there respective bits. This is the
   * inverse operation to split(...).
   *
