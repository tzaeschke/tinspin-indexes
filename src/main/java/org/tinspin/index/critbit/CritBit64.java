 ...
 *
 * <p>Version 1.3.4 - Merged with CB64COW - Implemented resetable iterators
 *
 * <p>Version 1.3.2 - Improved mask checking in QueryWithMask
 *
 * <p>Version 1.3.1 - Fixed issue #3 where iterators won't work with 'null' as values.
 *
 * <p>Version 1.3 - Removed separate field for prefix
 *
 * <p>Version 1.2.2 - simplified updateParentAfterRemove() - removed Node.posFirstBit - Moved test
 * to tst folder
 *
 * <p>Version 1.2.1 - Replaced compare() with '==' where possible - Simplified compare(),
 * doesInfixMatch(), get(), contains() - Replaced setBit() with set0()/set1()
 *
 * <p>Version 1.1: - added queryWithMask()
 *
...
public class CritBit64<V> implements Iterable<V> {
  ...
  private void printNode(Node<V> n, StringBuilder s, String level, int currentDepth, long infix) {
    ...
    if (currentDepth != n.posDiff) {
      s.append(
          level + "n: " + currentDepth + "/" + n.posDiff + " " + BitTools.toBinary(infix, 64) + NL);
    } else {
  ...
  private void updateParentAfterRemove(
      Node<V> parent, long newPost, V newVal, Node<V> newSub, boolean isParentHigh) {
  ...
  /**
   * Queries the tree for entries with {@code min<=key<=max}. Unlike the normal query, this query
   * also excludes all elements with {@code (key|min)!=key} and {@code (key&max!=max)}. See PH-Tree
   * for a discussion.
   *
   * @param min minimum key
   * @param max maximum key
