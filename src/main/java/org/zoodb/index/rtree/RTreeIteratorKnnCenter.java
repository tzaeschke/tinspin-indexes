...
public class RTreeIteratorKnnCenter<T> implements Iterator<DistEntry<T>> {
	...
	public RTreeIteratorKnnCenter(RTree<T> tree, double[] center, int k, 
			DistanceFunction dist) {
		this.stack = new IteratorStack(tree.getDepth());
		this.tree = tree;
		this.min = new double[tree.getDims()];
		this.max = new double[tree.getDims()];
		...
		while (!stack.isEmpty()) {
			...
			while (ip.pos < entries.size()) {
				...
				if (Entry.checkOverlap(min, max, e)) {
					//TODO this only works for EDGE distance
